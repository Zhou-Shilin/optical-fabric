...
public class FocusingRecipeGen extends ProcessingRecipeGen {
    ...
    GeneratedRecipe
            MIRROR = create(() -> CreateOptical.loc("mirror"), f -> {
                return (FocusingRecipeBuilder) f.require(ConventionalItemTags.GLASS_PANES).output(COItems.MIRROR).duration(50);
            }, FocusingRecipeParams.BeamTypeCondition.VISIBLE),
            FILTER = create(() -> CreateOptical.loc("filter"), f -> {
